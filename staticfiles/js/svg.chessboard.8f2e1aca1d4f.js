...
var MINIMUM_JQUERY_VERSION = '1.7.0',
  ...
  if (options.hasOwnProperty('position') === true) {
    if (options.position === 'start') {
      CURRENT_POSITION = deepCopy(START_POSITION);
    }

    else if (validFen(options.position) === true) {
      CURRENT_POSITION = fenToObj(options.position);
    }

    else if (validPositionObject(options.position) === true) {
      CURRENT_POSITION = deepCopy(options.position);
    }

    else {
      error(7263, 'Invalid value passed to config.position.', options.position);
    }
  }
...
function setCurrentPosition(position) {
  ...
  if (options.hasOwnProperty('onChange') === true &&
    typeof options.onChange === 'function') {
    options.onChange(oldPos, newPos);
  }
